...
- Зареєстровано 155 source-backed маршрутів із 31 рівня по 5 уроків; перші 34 — редакторська ціль першої хвилі, а не вимога вигадувати сторінки.
- Рівні 1–31 перенесено в авторські структуровані конспекти з прикладами, кроками, таблицями та source links.
- Усі 31 рівень мають source-backed metadata та authored content; levels 26–29 покривають legacy discovery, modernization, migration discovery, changelog analysis, dependency/compatibility analysis, pilot execution controls, rollback, behavior parity, migration types і data/configuration sequencing, level 30 — AI-native MVP-мислення, а level 31 — controlled vibe coding, implementation sprint, demo gates, readiness і capstone handoff.
- Level 27 додає сім reusable documented-example artifacts, level 28 — пʼять migration planning artifacts, level 29 — migration execution-control artifacts, level 30 — `VALUE_PROPOSITION.md`, `USER_JTBD.md`, `SUCCESS_METRIC.md` і `RELEASE_SLICE.md`, а level 31 — `CONTROLLED_VIBE_CODING.md`, `IMPLEMENTATION_SPRINT_PLAN.md`, `DEMO_QUALITY_GATE.md`, `DEMO_READINESS.md` і `CAPSTONE_DEFENSE_HANDOFF.md`, `DEMO.md`, `EVIDENCE_LOG.md` і `HANDOFF_PACKAGE.md`; загалом catalog містить 76 artifact records.
...
- Доступний site-wide пошук на [`/search`](/search): build-time projection індексує 155 уроків, 76 артефактів і 24 постановки задач, а браузер виконує deterministic lexical search без backend/API/database/persistence або зовнішнього search service.
...
- Окрема сторінка [`/tasks`](/tasks) містить 24 source-backed типи задач із заповненими прикладами `TASK_SPEC` та повʼязаних артефактів, що посилаються на всі 31 рівень; level 31 розширює capstone workflow без нового task type. Це documented examples, а не production evidence.
