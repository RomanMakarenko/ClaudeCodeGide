...
- Зареєстровано 150 source-backed маршрутів із 30 рівнів по 5 уроків; перші 34 — редакторська ціль першої хвилі, а не вимога вигадувати сторінки.
- Рівні 1–30 перенесено в авторські структуровані конспекти з прикладами, кроками, таблицями та source links.
- Усі 30 рівнів мають source-backed metadata та authored content; levels 26–29 покривають legacy discovery, modernization, migration discovery, changelog analysis, dependency/compatibility analysis, pilot execution controls, rollback, behavior parity, migration types і data/configuration sequencing, а level 30 — AI-native MVP-мислення, value proposition, user/JTBD, success metric, release slice і reviewer-ready specification.
- Level 27 додає сім reusable documented-example artifacts, level 28 — пʼять migration planning artifacts, level 29 — migration execution-control artifacts, а level 30 — `VALUE_PROPOSITION.md`, `USER_JTBD.md`, `SUCCESS_METRIC.md` і `RELEASE_SLICE.md`; загалом catalog містить 68 artifact records.
...
- Доступний site-wide пошук на [`/search`](/search): build-time projection індексує 150 уроків, 68 артефактів і 24 постановки задач, а браузер виконує deterministic lexical search без backend/API/database/persistence або зовнішнього search service.
...
- Окрема сторінка [`/tasks`](/tasks) містить 24 source-backed типи задач із заповненими прикладами `TASK_SPEC` та повʼязаних артефактів, що посилаються на всі 30 рівнів; це documented examples, а не production evidence.
